...
- **组合筛选**：类型、时间范围（近 7 天/30 天/90 天/近一年）、排序方式（时间/热度）可任意组合
- **条件记忆**：切换页面或回到首页后自动恢复上次的筛选组合，也可一键恢复默认
- **摘要视图**：在列表与摘要视图间切换，摘要展示当前结果的类型分布、浏览总量与热门 TOP3
- **空态恢复**：组合条件无结果时显示明确空态，可一键恢复默认条件
...
2. **组合筛选**：可同时选择类型、时间范围和排序方式，条件自动记忆
3. **切换视图**：点击"📊 摘要"查看当前结果的类型分布、浏览总量与热门 TOP3
4. **发布留言**：点击"发布留言"进入提交页面
5. **查看详情**：点击留言卡片查看完整内容
6. **恢复默认**：筛选无结果或想还原时，点击"↺ 恢复默认"清除全部条件
