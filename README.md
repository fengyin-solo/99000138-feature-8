...
- **排序筛选**：支持按类型、时间范围（今天/最近7天/最近30天）和时间/热度组合筛选，条件自动保留
- **结果摘要**：实时展示当前筛选结果的总数、分类分布与页码位置，无结果时可一键恢复默认
...
2. **组合筛选**：可同时选择类型、时间范围与排序方式，摘要栏实时概览当前结果
3. **条件保留**：翻页或离开后再回到首页，筛选条件自动保留；无结果时可一键恢复默认
